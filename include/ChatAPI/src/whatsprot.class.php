...
class WhatsProt
{
    ...
    public function connect()
    {
        ...
        $socket = fsockopen("e" . rand(1, 16) . ".whatsapp.net", Constants::PORT, $errno, $errstr, 30);
    ...
    public function pollMessage()
    {
        ...
        $w = array();
        $e = array();
        $s = stream_select($r, $w, $e, Constants::TIMEOUT_SEC, Constants::TIMEOUT_USEC);
...
//          $height = $json->height;
    ...
    {
        if ($this->isConnected()) {
            if (fwrite($this->socket, $data, strlen($data)) === false) {
              $this->eventManager()->fire("onClose",
                   array(
                        $this->phoneNumber,
                        "Connection closed!"
    ...
    * @param string $id
    */
    public function sendMessageRead($to, $id, $type = "read")
    {
	 ...
	 * @param string $type
     */
    public function sendGroupMessageRead($gid, $participant, $id, $type = "read")
    {
        $messageNode = new ProtocolNode("receipt",
            array(
                "type" => $type,
